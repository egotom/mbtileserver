...
func Slice (tc tileCoord,  src, dist string ) error{
	mz := math.Pow(2, float64(tc.z))
	ux := float64(tc.x) * circumference/mz-shift
	uy := float64(tc.y) * circumference/mz-shift
	lx := float64(tc.x+1) * circumference/mz-shift
	ly := float64(tc.y+1) * circumference/mz-shift  
	...
	adfGeoTransform[0] = ux
	adfGeoTransform[3] = uy
	
    adfGeoTransform[1] = (lx-ux)/ts
    adfGeoTransform[5] = (ly-uy)/ts
	...
	oDataset.IO(gdal.Write, wx, wy, wxSize, wySize, buffer, wxSize, wySize, rbc, aBand, nPixelSpace, nLineSpace, 0)
	return nil
}


func Slice2(tc tileCoord,  src, dist string ) error{
	ux,	uy, lx, ly := TileBounds(tc.x, tc.y, tc.z)
	fmt.Println(ux,uy,lx,ly)
	...
	ry := int((uy-adfGeoTransform[3])/adfGeoTransform[5]+0.001)
	rxSize := int((lx-ux)/adfGeoTransform[1] + 0.5)
	rySize := int((uy-ly)/adfGeoTransform[5] + 0.5)
	...
	oDataset.SetGeoTransform(adfGeoTransform);
	oDataset.SetProjection(iDataset.Projection());
	fmt.Println(iDataset.Projection(), nPixelSpace, nLineSpace)
	iDataset.IO(gdal.Read, rx, ry, rxSize, rySize, buffer, rxSize, rySize, rbc, aBand, nPixelSpace, nLineSpace, 0)
	oDataset.IO(gdal.Write, wx, wy, wxSize, wySize, buffer, wxSize, wySize, rbc, aBand, nPixelSpace, nLineSpace, 0)
	return nil
}
...
func GoogleTile(tx, ty, zoom int64)(int64, int64){
        //Converts TMS tile coordinates to Google Tile coordinates"
        //coordinate origin is moved from bottom-left to top-left corner of the extent
        return tx, (int64(math.Pow(2, float64(zoom)))- 1) - ty
}

func QuadTree(tx, ty, zoom int64)string{
	//"Converts TMS tile coordinates to Microsoft QuadTree"
	quadKey := ""
	ty = (int64(math.Pow(2, float64(zoom)))- 1) - ty
	for i:=zoom; i>0 ; i--{
		var digit int = 0
		mask := 1 << (i - 1)
		if (int(tx) & mask) != 0{
