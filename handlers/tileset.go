...
func newTileset(svc *ServiceSet, filename, id, path string) (*Tileset, error) {
	...
	m.HandleFunc(path+"/tiles/download", ts.updateHandler)
	m.HandleFunc(path+"/tiles/end-download", updateEndHandler)
...
func (ts *Tileset) updateHandler(w http.ResponseWriter, r *http.Request) {
	//body, _ := ioutil.ReadAll(r.Body)
	//fmt.Println(string(body))
	//fmt.Printf("###%+v", r.FormValue("x"))
	...
	if x >0 && y>0 && z>0{
		_, err := ts.get(z, x, y)
		if err != nil{
			res = `{"code":200,"message":"下载失败！","image":"----" }`
		}
	}
	fmt.Fprintf(w, res)
	// w.Write([]byte(res))
...
func updateEndHandler(w http.ResponseWriter, r *http.Request){
	fmt.Fprintf(w, `{"code":200,"message":"下载完成！"}`)
	return
}
