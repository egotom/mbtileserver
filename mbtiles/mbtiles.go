...
func Open(path string) (*MBtiles, error) {
	...
	pool, err := sqlitex.Open(path, sqlite.SQLITE_OPEN_READWRITE|sqlite.SQLITE_OPEN_NOMUTEX, 10)
	if err != nil {
		return nil, err
	}
...
func CreateDatabase(path string) error {
	// try to open file; fail fast if it exist
	_, err := os.Stat(path)
	if err == nil {
		if errors.Is(err, os.ErrNotExist) {
			return fmt.Errorf("path already exist: %q", path)
		}
		return err
	}
	fmt.Println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
	pool, err := sqlitex.Open(path, sqlite.SQLITE_OPEN_CREATE, 10)
	if err != nil {
		fmt.Println(err)
		return  err		
	}
...
// inserTile insert a tile for z, x, y into the provided *[]byte.
func (db *MBtiles) InserTile(z int64, x int64, y int64, data *[]byte) error {
	if db == nil || db.pool == nil {
		return errors.New("cannot insert tile to closed mbtiles database")
	}
	...
	query, err := con.Prepare("INSERT OR REPLACE INTO images (tile_id, tile_data) values ($id, $data)")
	if err != nil {
		return err
	}
	defer query.Reset()
	...
	stmt, err2 := con.Prepare("INSERT OR REPLACE INTO map  (zoom_level, tile_column, tile_row, tile_id)  values($z, $x, $y, $id)")
	if err2 != nil {
		return err
	}
	defer stmt.Reset()
...
// DeleteTile delete a tile identity by zoom_level = z, tile_column = x, tile_row = y.
func (db *MBtiles) DeleteTile(z int64, x int64, y int64) error {
	...
	query, err := con.Prepare("select tile_id from map where zoom_level=$z and tile_column=$x and tile_row=$y")
	if err != nil {
		return err
	}
	defer query.Reset()
	...
	stmt, err2 := con.Prepare("delete from  map where zoom_level=$z and tile_column=$x and tile_row=$y")
	if err2 != nil {
		return err
	}
	defer stmt.Reset()
	...
	if query.DataCount() == 1 {
		stmt, err2 = con.Prepare("delete from images where tile_id =$id ;")
		if err2 != nil {
			return err
		}
		stmt.SetText("$id", id)
